# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 15:55:26 2022

@author: Isabel Zuiker
"""
import math
...
os.chdir("C:\\Users\\Isabel Zuiker\\Documents\\Master\\Computer Science")
...
complete_data = cleandata(complete_data)
...
def pred_duplicates(complete_data, n_band, n_row, complete_modelwords):
    ...
    print(f'Threshold = {round(threshold,2)} where #b = {n_band} and #r = {n_row}')
...
def signature(mh_functions,v):
    index = np.nonzero(v)[0].tolist()  
    row_numbers = mh_functions[:, index]
...
def split_sign(signature, n_band, n_row):
    ...
    for i in range(0, len(signature), n_band):
        sub_signatures.append(signature[i : i+n_row])
    return sub_signatures

def hash_sign(signature, n_band, n_row):
    global counter
    global buckets
    sub_signatures = np.array(split_sign(signature, n_band, n_row))

    print(len(sub_signatures))
    print(len(buckets))
    for i,subsignature in enumerate(sub_signatures):
        subsignature = str(sub_signatures)
...
def clustering(d_matrix, t):
    clusters = AgglomerativeClustering(affinity='precomputed', linkage='average', distance_threshold=t, n_clusters=None)
...
plt.ylabel("PQ")
plt.title("Pairwise Quality vs Fraction of comparisons")
plt.show()
...
plt.xlabel("Fraction of Comparisons")
plt.ylabel("PC")
plt.title("Pairwise Completeness vs Fraction of comparisons")
plt.show()
